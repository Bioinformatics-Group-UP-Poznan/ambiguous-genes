...
for (dataset in datasets)
{
  ...
  dg.lists[[dataset]] <- rownames(dg$edgeR$stats[which(dg$edgeR$stats$status.group == "Sig" & dg$edgeR$stats$status.mappers == "Sig"),])
  ndg.lists[[dataset]] <- rownames(dg$edgeR$stats[which(dg$edgeR$stats$status.group == "Sig" & dg$edgeR$stats$status.mappers == "nonSig"),])
}
...
```{r tab-characteristics, results='asis'}
header <- list()
...
for (dataset in datasets)
{
  ...
  for (map in maper)
  {
    res.data1 <- readRDS(paste0("results/diffAna_",dataset, "_", map, ".rds"))
...
rownames(con.m) <- datasets1[match(rownames(con.m), names(datasets1))]
con.m <- con.m[order(rownames(con.m)),]
```
