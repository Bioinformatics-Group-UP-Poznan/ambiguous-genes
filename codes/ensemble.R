...
source("~/Documents/ADIUNKT/mapery-publikacja/kody/validation.R")
source("~/Documents/ADIUNKT/mapery-publikacja/kody/miscellaneous.R")
...
ensembleClassifier <- function(x, y, M=51, fit.individual=TRUE, varimp=FALSE, rfs=FALSE, nf=ceiling(sqrt(ncol(x))),
    ...
    for(k in 1:M){
        repeat{ # to make sure that at least two classes are present 
            ...
            if(length(unique(y[-s])) >= length(unique(y)) & length(unique(y[s])) >= length(unique(y)))
                break
        }
        ...
        for(j in 1:nalg){
            Res[[j]] <- switch(algorithms[j],
                ...
                "plr"      = penalized(trainY, penalized=training, trace=FALSE, model="logistic", lambda1=lambda1, ...),
        ...
        for(j in 1:nalg){
            switch(algorithms[j],
                ...
                "plr"       = {probabilities[[j]] <- predict(Res[[j]], testing)
                                temp <- probabilities[[j]] > .5
                                temp[temp] <- 1
                                predicted[[j]] <- factor(temp)},
			...
			if(!kalg %in% c("pls_lda", "pls_qda", "pls_rf", "pls_lr","pca_lda", "pca_qda", "pca_rf", "pca_lr")){
				...
				for(i in 1:(ncol(testing)+1)){
					...
					switch(kalg,
							...
							"plr"       = {temp <- predict(fittedModels[[k]], testing)
											temp <- temp > .5
											temp[temp] <- 1
											predicted[,i] <- factor(temp)},                     
    ...
    res <- list(models = fittedModels, indModels = Res, rawImportance=rawImportance, M = M, 
		bestAlg = bestAlg, levels=ly, importance=varImportance, convScores=convScores)
    class(res) <- "ensemble"
    res
}
            
predictEns <- function(EnsObject, newdata, y=NULL, plot=TRUE){
    M <- EnsObject$M
    n <- nrow(newdata)
    ...
    for(i in 1:M){
        ...
        testing <- newdata #[,EnsObject$models[[i]]$selectedFeatures]
        if(attr(EnsObject$models[[i]], "algorithm") %in% c("pls_lda", "pls_qda", "pls_rf", "pls_lr")){
            R <- attr(EnsObject$models[[i]], "R")
            meanX <- attr(EnsObject$models[[i]], "meanX")
            plsTestX <- scale(testing, scale=FALSE, center=meanX)%*%R
        }
        if(attr(EnsObject$models[[i]], "algorithm") %in% c("pca_lda", "pca_qda", "pca_rf", "pca_lr")){
            pcaTestX <- prcomp(testing)$x[,1:as.numeric(attr(EnsObject$models[[i]], "ncomp"))]
        }        
        
        switch(attr(EnsObject$models[[i]], "algorithm"),
                "svm"       = predicted[,i] <- predict(EnsObject$models[[i]], testing, prob=TRUE),
                "rf"        = predicted[,i] <- predict(EnsObject$models[[i]], testing, type="class"),
                "lda"       = predicted[,i] <- predict(EnsObject$models[[i]], testing)$class,
                "qda"       = predicted[,i] <- predict(EnsObject$models[[i]], testing)$class,
                "lr"        = {temp <- predict(EnsObject$models[[i]], data.frame(x=testing), type="response")
                                temp <- temp > .5
                                temp[temp] <- 1
                                predicted[,i] <- factor(temp)},
                "plr"       = {temp <- predict(EnsObject$models[[i]], testing)
                                temp <- temp > .5
                                temp[temp] <- 1
                                predicted[,i] <- factor(temp)},
                "pls_lda"   = predicted[,i] <- predict(EnsObject$models[[i]], plsTestX)$class,
                "pls_qda"   = predicted[,i] <- predict(EnsObject$models[[i]], plsTestX)$class,
                "pls_lr"    = {temp <- predict(EnsObject$models[[i]], data.frame(x=plsTestX), type="response")
                                temp <- temp > .5
                                temp[temp] <- 1
                                predicted[,i] <- factor(temp)},
                "pls_rf"    = predicted[,i] <- predict(EnsObject$models[[i]], plsTestX, type="class"),
                "pca_lda"   = predicted[,i] <- predict(EnsObject$models[[i]], pcaTestX)$class,
                "pca_qda"   = predicted[,i] <- predict(EnsObject$models[[i]], pcaTestX)$class,
                "pca_lr"    = {temp <- predict(EnsObject$models[[i]], data.frame(x=pcaTestX), type="response")
                                temp <- temp > .5
                                temp[temp] <- 1
                                predicted[,i] <- factor(temp)},
                "pca_rf"    = predicted[,i] <- predict(EnsObject$models[[i]], pcaTestX, type="class"),                         
                "nnet"      = predicted[,i] <- predictNNET(EnsObject$models[[i]], testing), # custom fn defined above
                "rpart"     = predicted[,i] <- predict(EnsObject$models[[i]], newdata=data.frame(testing), type="class"),
                "adaboost"  = predicted[,i] <- predict.boost(EnsObject$models[[i]], newdata=data.frame(testing))$class,
                "pls"       = predicted[,i] <- predict.pls(EnsObject$models[[i]], newdata=testing)$class
        )
    }
    ...
    if(length(EnsObject$indModels) > 0){  
       ...
       for(i in 1:length(EnsObject$indModels)){
            ...
            switch(attr(EnsObject$indModels[[i]], "algorithm"),            
                ...
                "plr"       = {indProb[,i] <- predict(EnsObject$indModels[[i]], testing)
                                temp <- indProb[,i] > .5
                                temp[temp] <- 1
                                indPred[,i] <- factor(temp)},
